...
def handle_negative(message):
    ...
    if len(rows) == 0:
        ...
        c.execute('Insert INTO message VALUES(NULL, ?, ?, ?, ?, ?, ?)', (message.message_id, message.text, 0,
                                                                         ...
                                                                         message.chat.id,))
        conn.commit()
        c.execute("SELECT MESSAGE_ID FROM message WHERE CREATED_AT >= Datetime('now', '-60 minutes', 'localtime') AND SENITMENT=0 AND USER_ID=?", (int(message.from_user.id)))
        rows = c.fetchall()
        print(rows)
        if len(rows) >= 5:
            bot.restrict_chat_member(message.chat.id, message.from_user.id, until_date=time()+60*settings.MINUTES)
            ...
            c.execute("INSERT INTO banned VALUES(NULL, ?, Datetime('now'), Datetime('now', '+180 minutes', 'localtime'))", int(message.from_user.id))
            conn.commit()
            #bot.delete_message(message.chat.id, message.message_id)
